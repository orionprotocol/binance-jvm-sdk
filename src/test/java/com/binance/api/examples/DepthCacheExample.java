...
public class DepthCacheExample {
  ...
  private List<DepthEvent> startDepthEventStreaming() {
    ...
    final WebSocket webSocketOpened = wsClient.onDepthEvent(symbol.toLowerCase(), wsCallback);
    this.webSocket = () -> {
      final int code = 1000;
      webSocketOpened.close(code, null);
    };
  ...
  private final class WsCallback implements WebSocketCallback<DepthEvent> {
